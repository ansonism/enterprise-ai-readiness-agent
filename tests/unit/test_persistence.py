...
from ai_readiness.models import ReadinessAssessmentInput, RunState
from ai_readiness.persistence import JsonRunStateRepository


def state() -> RunState:
    return RunState(
        run_id="safe-run_01",
        request=ReadinessAssessmentInput(case_id="case", title="Title", description="Description"),
    )
...
def test_json_repository_retries_transient_replace_lock(tmp_path: Path) -> None:
    ...
    with patch("ai_readiness.persistence.os.replace", side_effect=flaky_replace):
