...
from ai_readiness.models import ReadinessAssessmentInput, RiskAssessment, RunState, StageResult
from ai_readiness.observability import log_event
from ai_readiness.providers.mock import MockProvider


def test_models_reject_unknown_fields_and_invalid_probability() -> None:
    with pytest.raises(ValidationError):
        ReadinessAssessmentInput(case_id="x", title="t", description="d", unexpected=True)
    with pytest.raises(ValidationError):
...
def test_run_state_rejects_duplicate_stage_names() -> None:
    ...
    with pytest.raises(ValidationError, match="duplicate"):
        RunState(
            request=ReadinessAssessmentInput(case_id="x", title="t", description="d"),
...
def test_structured_log_redacts_sensitive_fields(caplog: pytest.LogCaptureFixture) -> None:
    with caplog.at_level(logging.INFO, logger="ai_readiness"):
