...
from ai_readiness.config import load_config
...
def test_environment_override_is_typed() -> None:
    settings = load_config(
        DEFAULT_CONFIG,
        environment={
            "AI_READINESS__PROVIDER__MAX_RETRIES": "5",
            "AI_READINESS__TOOLS__ALLOW_MUTATIONS": "false",
        },
    )
...
def test_unsafe_approval_configuration_is_rejected() -> None:
    with pytest.raises(ValidationError, match="Mutations cannot be enabled"):
        load_config(
            DEFAULT_CONFIG,
            environment={
                "AI_READINESS__TOOLS__ALLOW_MUTATIONS": "true",
                "AI_READINESS__APPROVAL__MODE": "disabled",
            },
        )
